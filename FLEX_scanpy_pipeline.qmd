...
poms_dict = {
    ...
    "BC003_POMS_01Pool2LNSF_G_1stpilot" : "POMS_025",
...
### updating sample names now that I know who is who

```{python}
flex_adata.obs["poms_id"] = flex_adata.obs["donor"].map(
    {
        "BC001":"POMS074",
        "BC002":"POMS075",
        "BC003":"POMS076",
...
```{python}
flex_adata.obs["sample_id"] = (flex_adata.obs["poms_id"].astype(str) + "_" + flex_adata.obs["pool_id"].astype(str))
flex_adata.obs
flex_adata.obs.groupby(['sample_id']).size()
```

### make QC plots again with updated names
```{python}
cells_per_sample = flex_adata.obs[['sample_id', "poms_id"]].value_counts().reset_index()
plt.figure(figsize=(10,5))
sns.barplot(data=cells_per_sample,
    x="sample_id",
    y="count",
    hue="poms_id"
)
...
sc.pl.umap(
    ...
    size = 20, legend_fontsize="x-large", ncols=1, wspace=0.5
)
plt.tight_layout()
...
split_umap(flex_adata, color="poms_id", split_by="pool_id", ncol=3)
...
### compare cluster 12 to all else
```{python}
sc.tl.rank_genes_groups(flex_adata, groupby="leiden_res_1", method="wilcoxon", key_added="dea_12vRest", groups=["12"], reference="rest")
```

### compare 12 to two of the most dominant clusters with opposing expression

```{python}
subset = flex_adata[flex_adata.obs["leiden_res_1"].isin(["1", "5", "12"])].copy()

sc.tl.rank_genes_groups(
    subset,
    groupby="leiden_res_1",
    groups=["12"],
    reference="rest",   # now rest = 1 + 5
    method="wilcoxon",
    key_added="dea_12v1and5"
)
sc.pl.rank_genes_groups_dotplot(
    subset, groupby="leiden_res_1", standard_scale="var", n_genes=10, key="dea_12v1and5",
)
```
...
plt.figure(figsize=(15,10))
sc.pl.rank_genes_groups_dotplot(
    flex_adata, groupby="leiden_res_1", standard_scale="var", n_genes=10, key="dea_12vRest",
)
plt.tight_layout()
plt.savefig("resolution_1_cluster_12_vs_rest_dotplot.png", dpi=300, bbox_inches="tight")
```
...
```{python}
plt.figure(figsize=(10,10))
sc.pl.dotplot(flex_adata, var_names=immune_genes, groupby="leiden_res_1")
plt.tight_layout()
plt.savefig("resolution_1_immune_genes_dotplot.png", dpi=300, bbox_inches="tight")
```

### looking at just immune clusters to then compare processing methods
```{python}
immune_adata = flex_adata[flex_adata.obs["leiden_res_1"].isin(["16", "18"])].copy()
immune_adata
```

```{python}
plt.figure(figsize=(6,5))
sc.pl.dotplot(immune_adata, var_names=immune_genes, groupby="sample_id")
plt.tight_layout()
plt.savefig("immune_clusters_expression_by_sample_dotplot.png", dpi=300, bbox_inches="tight")
```

### plot all cluster degs
```{python}
plt.figure(figsize=(20,5))
dp = sc.pl.rank_genes_groups_dotplot(
    flex_adata, groupby="leiden_res_1", standard_scale="var", n_genes=5, key="dea_leiden_1",show=False
)
ax = dp["mainplot_ax"]   # ← this is the real axis
ax.tick_params(axis="x", labelsize=20)
ax.tick_params(axis="y", labelsize=20)

plt.tight_layout()
plt.savefig("leiden_res_1_degs_dotplot.png", dpi=300, bbox_inches="tight")
```
