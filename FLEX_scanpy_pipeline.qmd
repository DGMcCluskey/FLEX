...
# DOUBLET REMOVAL
### this should be done on samples individually, which scDblFinder is capable of doing if you give it a sample column, so a merged object is ok
### using scDblFinder (an R package)


### i need to save anndata file then load it into R using Seurat's functions (where it will be a seurat object)
### i then convert seurat object into sce for use with scdblfinder
### after that i convert back into anndata
...
```{python}
sc.pl.highly_variable_genes(flex_adata)
```
