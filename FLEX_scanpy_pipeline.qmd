...
```{python}
sc.pl.violin(flex_adata, keys="pct_counts_mt", groupby="sample_id", stripplot=False, rotation=90, palette = ["mediumseagreen", "darkorchid", "orange", "grey"])
mean_per_sample = flex_adata.obs.groupby("sample_id")["pct_counts_mt"].mean()
print(mean_per_sample)
plt.figure()
sns.barplot(mean_per_sample)
plt.xticks(rotation=90)
plt.tight_layout()
cells_per_sample = flex_adata.obs.groupby("sample_id").size()
cells_per_sample
plt.figure()
sns.barplot(cells_per_sample)
plt.xticks(rotation=90)
plt.tight_layout()
```